...
function getRoundOverText() {
	return '<div class = centerbox><p class = center-block-text>' + round_over_text +
		' 本轮已结束。</p><p class = center-block-text>按 <strong>Enter</strong> 键开始。</p></div>'
}
...
function getGame() {
	...
	*/
	if (total_fish_num === 0) {
		...
		game_state = appendTextAfter(game_state, '单次银行（积分）: </strong>', trip_bank)
		game_state = appendTextAfter(game_state, '锦标赛银行: </strong>', tournament_bank)
		game_state = appendTextAfter(game_state, '保温箱中的红鱼: </strong>', 0)
		game_state = appendTextAfter(game_state, "钓鱼 ", release)
		...
		game_state = appendTextAfter(game_state, "weathertext>", weather)
		if (release == "保留") {
			game_state = appendTextAfter(game_state, '保温箱中的红鱼: </strong>', trip_bank)
		}
		$('.jspsych-display-element').html(game_state)
		if (weather == "晴天") {
			$('.lake').css("background-color", "LightBlue")
	...
	*/
	if (total_fish_num > 0) {
		if (Math.random() < 1 / (total_fish_num)) {
			...
			round_over = 1
			round_num += 1
			round_over_text = "您抓到了蓝鱼！您失去了本轮收集的所有积分。"
		} else {
			if (release == "保留") {
...
function collect() {
	round_over = 1
	round_num += 1
	round_over_text = "您从单次银行收集了 " + trip_bank +
		" 积分并将其转移到您的锦标赛银行。"
	...
	$('#trip_bank').html('<strong>单次银行（积分）:</strong> ' + trip_bank)
	red_fish_num = 0
	total_fish_num = 0
	lake_state = $('.lake').html()
...
var game_setup = "<div class = titlebox><div class = center-text>钓鱼 </div></div>" +
	"<div class = lake></div>" +
	"<div class = cooler><p class = info-text>&nbsp<strong>保温箱中的红鱼: </strong></p></div>" +
	...
	"</div>" +
	"<div class = subinfocontainer>" +
	"<div class = infobox><p class = info-text id = trip_bank><strong>单次银行（积分）: </strong></p></div> " +
...
var feedback_instruct_text =
	'欢迎参加实验。该实验将耗时20分钟左右。按 <strong>Enter</strong> 键开始。'
...
var instructions_block = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>在这个任务中，您将参加一个钓鱼锦标赛。在这个锦标赛期间，您将进行多轮钓鱼游戏。每一轮，您将看到一个湖泊，里面有很多鱼。您的目标是尽可能多地抓鱼。</p><p class = block-text>在屏幕上您将看到一个湖泊和两个按钮：“开始钓鱼”和“收集积分”。如果您点击“开始钓鱼”，您将随机抓到湖中的一条鱼。每条鱼被抓到的机会相等。</p><p class = block-text>湖中有许多红鱼和一条蓝鱼。每条红鱼可以为您赚得1分，这些分数进入当轮的“单次银行”，您可以通过“收集积分”将分数移动到“锦标赛银行”并开始新一轮。然而，如果您抓到蓝鱼，本轮将结束，您将失去本轮赚得的所有分数。</p><p class = block-text>要保持您在各轮之间的分数，您必须在抓到蓝鱼之前停止钓鱼并按下“收集积分”。</p><p class = block-text>结束说明后，您将进行 ' + num_practice_rounds + ' 轮钓鱼游戏练习。</p></div>'
	],
	allow_keys: false,
	data: {
		trial_id: 'instruction'
	...
	loop_function: function(data) {
		...
		if (sumInstructTime <= instructTimeThresh * 1000) {
			feedback_instruct_text =
				'您阅读说明太快了。请花时间仔细阅读并确保您理解说明。按 <strong>Enter</strong> 键继续。'
			return true
		} else if (sumInstructTime > instructTimeThresh * 1000) {
			feedback_instruct_text = '说明已完成。按 <strong>Enter</strong> 键继续。'
			return false
		}
	}
}
...
var conditions_instructions_block = {
	type: 'poldrack-instructions',
	pages: [
		'<div class = centerbox><p class = block-text><p class = block-text>您将参加两个锦标赛，每个都有不同的规则。这些锦标赛的区别在于您是保留还是释放所抓到的鱼。在 <span style="color:red">释放钓法</span> 条件下，您将总是释放刚刚抓到的鱼，所以红鱼和蓝鱼的数量在整轮中保持不变。</p><p class = block-text>在 <span style="color:red">保留钓法</span> 条件下，您抓到的鱼将从湖中移出并放入您的保温箱。因此，每次您抓到红鱼时，抓到蓝鱼的机会都会增加。</p></div>',
		'<div class = centerbox><p class = block-text>每个锦标赛都是独立的。您在一个锦标赛中赚得的分数对下一个没有影响。您的目标是在两个锦标赛中都尽可能做得好。</p><p class = block-text>您可以通过在任务中表现良好来赚取奖励金，所以请尽力最大化您的收益！您的奖励金将与您的收益成正比。</p></div>',
		'<div class = centerbox><p class = block-text>在我们开始锦标赛之前，每个锦标赛都将有一个简短的练习环节。在每个练习锦标赛开始之前，您将选择湖中鱼的数量（1-200）。在实际实验中，您将无法选择鱼的数量。</p></div>'
	],
	allow_keys: false,
	data: {
		trial_id: 'instruction'
...
var end_block = {
	type: 'poldrack-text',
	text: '<div class = centerbox><p class = center-block-text>感谢您完成此任务！</p><p class = center-block-text>按 <strong>Enter</strong> 键结束。</p></div>',
	cont_key: [13],
	data: {
		trial_id: "end",
...
var ask_fish_block = {
	...
	questions: [
		[
			"<p class = center-block-text>在这个锦标赛中，湖里有多少条红鱼？请输入1-200之间的数字</p><p class = center-block-text>如果您不回答或回答超出此范围，红鱼的数量将在1-200之间随机设定。</p>"
		]
	],
}
...
var start_test_block = {
	...
	timing_response: 180000,
	text: '<div class = centerbox><p class = center-block-text>练习完成！我们现在开始正式锦标赛。将有四个锦标赛，每个都有30轮钓鱼。</p><p class = center-block-text>按 <strong>Enter</strong> 键开始测试。</p></div>',
	cont_key: [13],
	timing_post_trial: 1000,
	on_finish: function() {
		tournament_bank = 0
...
weather = "晴天"
release = "保留"
release_rule = "您抓到的鱼会从湖中移出"
var tournament_intro_block_practice = {
	type: 'poldrack-text',
	text: '<div class = centerbox><p class = block-text>您现在将开始一个锦标赛。释放规则是 <span style="color:red">' + release + '</span>，这意味着 ' +
		release_rule +
