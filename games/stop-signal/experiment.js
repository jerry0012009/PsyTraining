...
var getTestFeedback = function() {
	...
	test_feedback_text = "<br>完成了一个测试块。请花时间阅读您的反馈并稍作休息！阅读反馈后请按<strong>enter</strong>键继续。"
	test_feedback_text += "</p><p class = block-text><strong>平均反应时间：  " + Math.round(average_rt) + " 毫秒。非星形试次的准确率： " + Math.round(GoCorrect_percent * 100)+ "%</strong>" 
	if (average_rt > RT_thresh || rt_diff > rt_diff_thresh) {
		test_feedback_text +=
			'</p><p class = block-text>您的反应太慢了，请尽可能快速准确地对每个形状做出反应。'
	}
	if (missed_responses >= missed_response_thresh) {
		test_feedback_text +=
			'</p><p class = block-text><strong>我们检测到一些需要反应的试次中没有做出反应。请确保您对每个形状做出反应，除非出现星形。</strong>'
	}
	if (GoCorrect_percent < accuracy_thresh) {
		test_feedback_text += '</p><p class = block-text>您的准确率太低。请记住，正确的按键如下： ' + prompt_text
	}
	if (StopCorrect_percent < (0.5-stop_thresh) || stopAverage < 0.45){
			 	test_feedback_text +=
			 		'</p><p class = block-text><strong>记住在看到停止信号时要尽力停止反应。</strong>'	
	} else if (StopCorrect_percent > (0.5+stop_thresh) || stopAverage > 0.55){
	 	test_feedback_text +=
	 		'</p><p class = block-text><strong>请记住，不要为了看星形是否会出现而放慢您对形状的反应。请尽可能快速准确地对每个形状做出反应。</strong>'
	}
...
var feedback_instruct_text =
	'欢迎参加实验。这个实验大约需要 30 分钟。按<strong>enter</strong>键开始。'
...
var instructions_block = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>在这个任务中，您将看到黑色形状逐个出现在屏幕上。您需要通过按"Z"和"M"键来对它们做出反应。</p></div>',
		'<div class = centerbox><p class = block-text>每个形状只有一个正确的按键。正确的按键如下：' + prompt_text +
		'</p><p class = block-text>这些指导在练习阶段会保留在屏幕上，但在测试阶段会被移除。</p><p class = block-text>您应尽可能快速准确地对每个形状做出反应。</p></div>',
	],
	allow_keys: false,
	...
	loop_function: function(data) {
		...
		if (sumInstructTime <= instructTimeThresh * 1000) {
			feedback_instruct_text =
				'阅读指导语太快了。请花时间确保您理解这些指导语。按<strong>enter</strong>键继续。'
			return true
		} else if (sumInstructTime > instructTimeThresh * 1000) {
			feedback_instruct_text = '指导语完成。按<strong>enter</strong>键继续。'
			return false
		}
	}
}
...
var practice_feedback_text =
	'现在我们将开始练习部分。在练习中，请专注于对每个形状快速准确地做出反应。按<strong>enter</strong>键继续。'
...
var NoSS_practice_node = {
	...
	loop_function: function(data) {
		...
		var GoCorrect_percent = sum_correct / go_length;
		var missed_responses = (go_length - num_responses) / go_length
		practice_feedback_text = "</p><p class = block-text><strong>平均反应时间：  " + Math.round(average_rt) + " 毫秒。非星形试次的准确率： " + Math.round(GoCorrect_percent * 100)+ "%</strong>" 
		if ((average_rt < RT_thresh && GoCorrect_percent > accuracy_thresh && missed_responses <
			...
			practice_feedback_text +=
				'</p><p class = block-text>在实验的其余部分，在一定比例的试次中，会在形状周围出现一个星形的黑色"停止信号"。当这种情况发生时，请尽您最大的努力停止您的反应，在该试次中不按任何键。</p><p class = block-text>星形会在形状出现的同时或稍后出现。因此，当星形出现时，您并不总是能成功停止。然而，如果您继续非常努力地在星形出现时停止，您将能够有时停止成功，但不是总是如此。</p><p class = block-text><strong>请平衡对形状快速准确反应的要求，同时非常努力地对停止信号做出停止反应。</strong></p><p class = block-text>按<strong>Enter</strong>键继续'
			return false;
		} else {
			//rerandomize stim order
			NoSS_practice_list = jsPsych.randomization.repeat(stimulus, 3, true)
			// keep going until they are faster!
			practice_feedback_text += '</p><p class = block-text>我们将尝试另一个练习块。 '
			...
			if (missed_responses >= missed_response_thresh) {
				practice_feedback_text +=
					'</p><p class = block-text><strong>我们检测到一些需要反应的试次中没有做出反应。请确保您对每个形状做出反应。</strong>'
			}
			if (GoCorrect_percent <= accuracy_thresh) {
				practice_feedback_text +=
					'</p><p class = block-text>您的准确率太低。请记住，正确的按键如下： ' + prompt_text
			}
			practice_feedback_text += '</p><p class = block-text>按<strong>Enter</strong>键继续'
			return true;
		}
	}
}
...
var practice_node = {
	...
	loop_function: function(data) {
		...
		if ((average_rt < RT_thresh && GoCorrect_percent > accuracy_thresh && missed_responses <
		    ...
		    current_trial = 0
			practice_feedback_text +=
				'</p><p class = block-text>练习完成。现在我们将开始 ' + numconditions *
				numblocks +
				' 个测试块。每个块后都会有休息。按<strong>enter</strong>键继续。'
			return false;
		} else {
