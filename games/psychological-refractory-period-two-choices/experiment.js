  ...
  if (rts[0] !== -1 && rts[1] === -1) {
    if (jQuery.inArray(keys[0], choices1) === -1) {
      ...
      if (keys[0] === data.choice2_correct_response) {
        choice2FB = '您对数字的反应是正确的！'
      } else {
        choice2FB = '您对数字的反应不正确。请记住：如果数字是 ' +
          inners[0] + ' 请按 "N" 键。如果数字是 ' + inners[1] +
          ' 请按 "M" 键。'
      }
    } else if (jQuery.inArray(keys[0], choices2) === -1) {
      choice2FB = '您没有对数字做出反应！'
      if (keys[0] === data.choice1_correct_response) {
        choice1FB = '您对彩色方块的反应是正确的！'
      } else {
        choice1FB = '您对彩色方块的反应不正确。请记住：如果方块是 ' + borders[0][1] + ' 请按 "Z" 键。如果方块是 ' + borders[1][1] + ' 请按 "X" 键。'
      }
    }
    ...
    choice1FB = '请对方块和数字做出反应！'
  }
  if (tooShort) {
    return '<div class = prp_centerbox><p class = "center-block-text">您在数字出现在屏幕上之前就按了 "N" 或 "M" 键！请等数字出现后再反应！</p><p class = "center-block-text">按任意键继续</p></div>'
  } else {
...
var post_task_block = {
   ...
   questions: ['<p class = center-block-text style = "font-size: 20px">请总结一下您在这个任务中被要求做什么。</p>',
              '<p class = center-block-text style = "font-size: 20px">您对这个任务有什么评价吗？</p>'],
   rows: [15, 15],
   columns: [60,60]
};
...
var end_block = {
  ...
  text: '<div class = prp_centerbox><p class = "center-block-text">感谢您完成这个任务！</p><p class = "center-block-text">按 <strong>回车键</strong> 继续。</p></div>',
  cont_key: [13],
  timing_post_trial: 0,
...
var feedback_instruct_text =
  '欢迎参加实验。这个实验将持续大约 12 分钟。按 <strong>回车键</strong> 开始。'
...
var instructions_block = {
  ...
  pages: [
    '<div class = prp_centerbox><p class ="block-text">在这个实验中，您需要快速连续完成两个任务。您需要用双手的食指和中指按下 "Z"、"X"、"N" 和 "M" 键来反应。</p><p class ="block-text">首先，屏幕上将出现一个彩色方块。如果是下面左侧的 ' + borders[0][1] + ' 方块，您应该按 "Z" 键。如果是右侧的 ' + borders[1][1] + ' 方块，您应该按 "X" 键。</p>' +
    box1 + box2 + '</div>',
    '<div class = prp_centerbox><p class ="block-text">短暂延迟后，将在方块中出现两个数字中的一个（如下所示）。如果数字是 ' +
    inners[0] + ' 请按 "N" 键。如果数字是 ' + inners[1] +
    ' 请按 "M" 键。</p><p class ="block-text">非常重要的是您要尽可能快地反应！您应该先对彩色方块反应，然后再对数字反应。尽可能快地对彩色方块反应，然后对数字反应。</p>' +
    box_number1 + box_number2 + '</div>', '<div class = prp_centerbox><p class ="block-text">在您结束说明后，我们将先进行一些练习。请确保您记住要对哪些彩色方块反应，以及对两个数字要按哪些键，然后再继续。如果需要的话，请再次查看说明。</p></div>'
  ],
  allow_keys: false,
  ...
  loop_function: function(data) {
    ...
    if (sumInstructTime <= instructTimeThresh * 1000) {
      feedback_instruct_text =
        '阅读说明太快了。请花时间仔细阅读，确保您理解了说明。按 <strong>回车键</strong> 继续。'
      return true
...
var start_practice_block = {
  ...
  text: '<div class = prp_centerbox><p class = "center-block-text">我们将开始 ' +
    practice_len + ' 次练习试验。按 <strong>回车键</strong> 开始。</p></div>',
  cont_key: [13],
  timing_post_trial: 1000
};
...
var start_test_block = {
  ...
  text: '<div class = prp_centerbox><p class ="center-block-text">现在我们开始正式测试。按 <strong>回车键</strong> 开始。</p></div>',
  cont_key: [13],
  timing_post_trial: 1000,
  on_finish: function() {
    current_trial = 0
