...
Game.Instructions.prototype = {
  ...
  instruct: function(num) {
    txt_style = {font: "20px Arial", fill: "#FFFFFF", align: "center"}
    if (num == 1) {
      this.back_ground = this.makeBox(220,400,500,190)
      txt = "在这个游戏中，您将看到三个网格\n它们有多色方块。"
      instruct_text = this.game.add.text(this.game.world.centerX+175,425,txt,txt_style)
      ...
      this.back_ground = this.makeBox(220,400,500,190)
      txt = "您的任务是判断左边两个网格中的方块\n是否与右边网格中的方块\n位置相同。"
      instruct_text = this.game.add.text(this.game.world.centerX+175,425,txt,txt_style)
      instruct_text.anchor.x = (0.5,0.5)
      this.nextB = this.add.button(475, 525, 'next', function () {
        this.game.world.remove(instruct_text)
        this.rmInstructB = true
      }, this)
      this.inst_finished = false
      ...
      instruct_text = this.game.add.text(this.game.world.centerX+175,425,txt,txt_style)
      instruct_text.anchor.x = (0.5,0.5)
      this.nextB = this.add.button(475, 475, 'next', function () {
        this.game.world.remove(instruct_text)
        this.rmInstructB = true
      }, this)
      this.inst_finished = false
  ...
  nextTrial: function () {
    ...
    this.trial++
    this.progress = this.game.add.text(860, 560, this.trial + ' / ' + this.op1s.length, {font:'30px Arial', fill:'#FFFFFF', align:'center'})
  ...
  endTrial: function () {
    ...
    if (this.streak == 3) {
      ...
      corrFeedback = '连续15个正确！'
      disp_col = '#3CF948'
    } else if (this.streak == 24) {
      corrFeedback = '完美得分！'
      disp_col = '#3CF948'
    } else {
      correct_feedback = ['太棒了！','好极了！','你真厉害！','正确！','太好了！','很好！']
    ...
    if (this.trial >= this.op1s.length && this.answer == "correct") {
      ...
      if (this.answer == "incorrect") {
        that = this
        setTimeout(function(){
          that.feedback = that.game.add.text(that.game.width/2-300, that.game.height/2+190, "很遗憾 :(", {font:'60px Arial', fill:disp_col, align:'center'})
          that.feedback.anchor.x = 0.5
        }, 500)
        this.numFeedback.visible = true
        this.game.world.remove(this.progress)
  ...
  quitGame: function () {
    ...
    this.game.time.events.add(Phaser.Timer.SECOND, function () {
      instructions = this.game.add.text(490, 250, '做得很好！理解了吗？如果是的话，您可以按"开始"按钮开始游戏。\n如果没有，您可以点击"返回"按钮重新阅读说明\n您将在主游戏中完成24个问题。', {font:'20px Arial', fill:'#FFFFFF', align:'center'});
