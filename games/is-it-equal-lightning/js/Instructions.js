...
Game.Instructions.prototype = {
  ...
  makeBox: function(x,y,width,height) {
    ...
    if (num == 1) {
          instruct_text = this.game.add.text(this.game.world.centerX-100,50,"在这个游戏中，你需要判断左边的算式\n是否等于右边的数字。",
          ...
          instruct_text = this.game.add.text(this.game.world.centerX-50,50,"如果它们相等，请按相等按钮\n如果它们不相等，请按不等按钮。",
          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
          this.nextB = this.add.button(this.game.world.centerX+175, 120, 'next', function () {
            this.game.world.remove(instruct_text)
            this.rmInstructB = true
          ...
          this.nextB.scale.setTo(0.5,0.5)
                  ...
                  instruct_text = this.game.add.text(this.game.world.centerX+100,50,"这个圆圈显示你剩余的时间。",
                  {font: "20px Arial", fill: "#FFFFFF", align: "center"})
                  ...
                  instruct_text = this.game.add.text(this.game.world.centerX+100,50,"真正的游戏会比这个快很多。",
                  {font: "20px Arial", fill: "#FFFFFF", align: "center"})
                  this.nextB = this.add.button(this.game.world.centerX+300, 80, 'next', function () {
                    this.game.world.remove(instruct_text)
                    this.rmInstructB = true
          ...
          this.game.world.remove(this.back_ground)
          this.instruct_text = this.game.add.text(this.game.world.centerX-40,80,"现在你可以回答这个问题了。",
          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
  ...
  nextTrial: function () {
    ...
    this.trial++
    this.progress = this.game.add.text(860, 560, this.trial + ' / 1', {font:'30px Arial', fill:'#FFFFFF', align:'center'})
    this.progress.anchor.x = 0.5

    this.pointDisplay = this.game.add.text(85, 560, '金币: ' + this.points, {font:'30px Arial', fill:'#FFFFFF', align:'center'})
  ...
  endTrial: function () {
    ...
    if (this.streak == 3) {
      corrFeedback = '连续3个正确！额外金币！'
      disp_col = '#3CF948'
    } else if (this.streak == 7) {
      corrFeedback = '连续7个正确！额外金币！'
      disp_col = '#3CF948'
    } else if (this.streak == 15) {
      corrFeedback = '连续15个正确！额外金币！'
      disp_col = '#3CF948'
    } else if (this.streak == 26) {
      corrFeedback = '完美得分！额外金币！'
      disp_col = '#3CF948'
    } else {
      correct_feedback = ['太棒了！','很好！','你太厉害了！','正确！','非常好！','不错！']
    ...
    if ((this.trial) >= this.op1s.length && this.answer == "correct" && !this.timedOut) {
      ...
      if (this.answer == "incorrect") {
        this.feedback = this.game.add.text(this.game.width/2, 50, "再试一次！", {font:'80px Arial', fill:disp_col, align:'center'})
        this.feedback.anchor.x = 0.5
        this.numFeedback.visible = true
        this.game.world.remove(this.progress)
        this.game.world.remove(this.pointDisplay)
        ...
        setTimeout(function() {
          ...
          that.makeProb()
          that.makeButtons()
          that.progress = that.game.add.text(860, 560, that.trial + ' / 1', {font:'30px Arial', fill:'#FFFFFF', align:'center'})
          that.progress.anchor.x = 0.5

          that.pointDisplay = that.game.add.text(85, 560, '金币: ' + that.points, {font:'30px Arial', fill:'#FFFFFF', align:'center'})
  ...
  quitGame: function () {
    ...
    //nextTask(this.results[0], this.task)
      ...
      //Let them know it's done...
        ...
        instructions = this.game.add.text(490, 50, '干得好！明白了吗？如果是的话，你可以按"开始"按钮开始游戏。\n如果没有，你可以点击"返回"按钮重复指示\n在正式游戏中，你需要完成24个问题。\n\n 记住，时间会比现在快得多！', {font:'20px Arial', fill:'#FFFFFF', align:'center'});
