...
Game.Instructions.prototype = {
  ...
  makeBox: function(x,y,width,height) {
    ...
    if (num == 1) {
          instruct_text = this.game.add.text(this.game.world.centerX-100,50,"在这个游戏中，你的任务是判断左边的算式\n是否等于右边的数字。",
          ...
          instruct_text = this.game.add.text(this.game.world.centerX-50,50,"如果相等，请按相等按钮\n如果不相等，请按不相等按钮。",
          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
          this.nextB = this.add.button(this.game.world.centerX+175, 120, 'next', function () {
            this.game.world.remove(instruct_text)
            this.rmInstructB = true
          ...
          this.instruct_text = this.game.add.text(this.game.world.centerX-40,70,"现在你可以给出这道题的答案了。",
          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
  ...
  endTrial: function () {
    ...
    if (this.streak == 3) {
      corrFeedback = '连续3个！额外奖励一枚金币！'
      disp_col = '#3CF948'
    } else if (this.streak == 7) {
      corrFeedback = '连续7个！额外奖励一枚金币！'
      disp_col = '#3CF948'
    } else if (this.streak == 15) {
      corrFeedback = '连续15个！额外奖励一枚金币！'
      disp_col = '#3CF948'
    } else if (this.streak == 24) {
      corrFeedback = '完美得分！额外奖励一枚金币！'
      disp_col = '#3CF948'
    } else {
      correct_feedback = ['太棒了！','太棒了！','你真棒！','正确！','太妙了！','不错！']
  ...
  quitGame: function () {
    ...
    //nextTask(this.results[0], this.task)
      ...
      //Let them know it's done...
        ...
        instructions = this.game.add.text(490, 50, '做得不错！明白了吗？如果是的话，你可以按"开始"按钮开始。\n如果还不清楚，你可以点击"返回"按钮重复指导\n在正式游戏中你需要完成24道题目。', {font:'20px Arial', fill:'#FFFFFF', align:'center'});
        ...
        this.game.world.sendToBack(this.back_ground)
        this.go = this.add.button(250, 225, 'go', function () {this.state.start('Run', true, false, 0, true, this.problem_set);}, this);
