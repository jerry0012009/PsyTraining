...
Game.Run.prototype = {
  ...
  nextTrial: function () {
    ...
    this.trial++
    this.progress = this.game.add.text(860, 560, this.trial + ' / 24', {font:'30px Arial', fill:'#FFFFFF', align:'center'})
    ...
    if ((this.trial) >= this.op1s.length && this.answer == "correct" && !this.timedOut) {
      ...
      if (this.answer == "incorrect") {
        ...
        setTimeout(function() {
          ...
          that.makeProb()
          that.makeButtons()
          that.progress = that.game.add.text(860, 560, that.trial + ' / 24', {font:'30px Arial', fill:'#FFFFFF', align:'center'})
  ...
  quitGame: function () {
    ...
    //nextTask(this.results[0], this.task)
      ...
      this.game.time.events.add(Phaser.Timer.SECOND, function () {
        endText = this.game.add.text(this.game.width/2, 200, '全部完成！', {font:'96px Arial', fill:'#FFFFFF', align:'center'});
        endText.anchor.x = 0.5
        finalPoints = this.game.add.text(this.game.width/2, 400, '你得到了' + this.points + '分', {font:'76px Arial', fill:'#FFFFFF', align:'center'});
