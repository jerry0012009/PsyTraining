...
var post_task_block = {
   ...
   questions: ['<p class = center-block-text style = "font-size: 20px">请简要总结你在这个任务中需要做什么。</p>',
...
var feedback_instruct_text =
	'欢迎参与实验。按下 <strong>回车键</strong> 开始。'
...
var instructions_block = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>在这个任务中，你需要使用方向键选择两个形状中的一个来赚取积分。下一页将展示一个示例试验。</p></div>',
		...
		'<div class = centerbox><p class = block-text>如你所见，每个形状都对应一个特定的数值（0-100之间）。两个形状的数值之和总是等于100。因此，如果蓝色形状的值是30，绿色形状的值就是70。</p><p class = block-text>当你选择一个形状时，如果你选择正确（下一页将解释），你将获得该形状上标明的积分。</p><p class = block-text>屏幕底部是一个红色进度条，显示你已获得的积分。如果进度条超过灰线，你将获得奖励。如果超过黄线，你将获得更大的奖励。</p></div>',
		'<div class = centerbox><p class = block-text>每个试验中正确的颜色形状是概率性的，就像掷硬币一样。正确颜色是蓝色或绿色的概率可能在实验过程中发生变化，但仅取决于最近的结果历史。因此，如果在过去几个试验中绿色大部分时间都是正确的，那么将来它也很可能是正确的。</p><p class = block-text>无论你选择哪种颜色，在你做出反应后，该试验的正确颜色都会显示出来。如果你选择了那个颜色，你将获得相应的积分。每种颜色的形状都可能出现在屏幕的任一侧，但位置不会影响该形状获胜的可能性。</p><p class = block-text>总结一下，每个试验中你需要选择蓝色或绿色形状中的一个。如果你选择正确，你将获得该形状上标明的积分并加到你的总分中。你的目标是尽可能多地获得积分。</p></div>'
	],
	allow_keys: false,
	...
	loop_function: function(data) {
		...
		if (sumInstructTime <= instructTimeThresh * 1000) {
			feedback_instruct_text =
				'阅读指令过快。请仔细阅读并确保理解指令内容。按下 <strong>回车键</strong> 继续。'
			return true
		} else if (sumInstructTime > instructTimeThresh * 1000) {
			feedback_instruct_text = '指令阅读完成。按下 <strong>回车键</strong> 继续。'
			return false
		}
	}
}
...
var end_block = {
	...
	text: '<div class = centerbox><p class = center-block-text>感谢您完成这个任务！</p><p class = center-block-text>按下 <strong>回车键</strong> 继续。</p></div>',
	cont_key: [13],
	timing_response: 180000,
	timing_post_trial: 0
};
...
var start_test_block = {
	...
	text: '<div class = centerbox><p class = center-block-text>开始测试阶段。按下 <strong>回车键</strong> 开始。</p></div>',
	cont_key: [13],
	timing_response: 180000,
	timing_post_trial: 1000
};
