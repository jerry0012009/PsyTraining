...
Game.Run.prototype = {
  ...
  create: function () {
    ...
    //first trial
    this.progress = this.game.add.text(this.game.world.centerX*2-100, 560, '第1题 共'+this.op1s.length+'题', {font:'30px Arial', fill:'#FFFFFF', align:'center'})
  ...
  nextTrial: function() {
    ...
    if (this.trial == 0) {
      this.progress.setText('第1题 共'+this.op1s.length+'题')
      this.pointDisplay.setText("金币：" + this.points)
    } else {
      this.progress.setText('第'+(this.trial+1)+'题 共'+this.op1s.length+'题')
      this.pointDisplay.setText("金币：" + this.points)
    }
    this.trial++
  ...
  quitGame: function () {
      ...
      this.game.time.events.add(Phaser.Timer.SECOND, function () {
        endText = this.game.add.text(this.game.width/2, 150, '全部完成！', {'font': '70px Arial', 'fill':'#fff'});
        endText.anchor.x = 0.5
        finalPoints = this.game.add.text(this.game.width/2, 250, '你获得了' + this.points + '分', {font:'70px Arial', fill:'#FFFFFF', align:'center'});
