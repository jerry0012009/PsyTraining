...
var confidence_response_area =
	...
	'<button class = response_button id = Confidence_1>1:<br> 完全不确定</button>' +
	'<button class = response_button id = Confidence_2>2</button>' +
	'<button class = response_button id = Confidence_3>3</button>' +
	'<button class = response_button id = Confidence_4>4:<br> 非常确定</button>'

var confidence_response_area_key =
	'<div class = centerbox><div class = fixation>+</div></div><div class = response_div>' +
	'<button class = response_button_key id = Confidence_1>1:<br> 完全不确定</button>' +
	'<button class = response_button_key id = Confidence_2>2</button>' +
	'<button class = response_button_key id = Confidence_3>3</button>' +
	'<button class = response_button_key id = Confidence_4>4:<br> 非常确定</button>'
...
var post_task_block = {
   ...
   questions: ['<p class = center-block-text style = "font-size: 20px">请总结一下您在这个任务中需要做什么。</p>',
...
var end_block = {
	...
	text: '<div class = centerbox><p class = center-block-text>谢谢您完成这个任务！</p><p class = center-block-text>按 <strong>Enter</strong> 键继续。</p></div>',
	cont_key: [13],
	timing_post_trial: 0,
...
var feedback_instruct_text =
	'欢迎参加本实验。实验大约需要 30 分钟。请按 <strong>Enter</strong> 键开始。'
...
var instructions_block = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>在这个实验中，您将看到两块随机噪声。在其中一块中有一个轻微的栅格（条纹图案）。您的任务是使用箭头键指示左侧还是右侧刺激具有栅格。屏幕中央有一个注视十字。由于刺激出现在屏幕上的时间非常短，所以请将目光集中在十字上。</p><p class = block-text>在您做出选择后，系统会要求您评估您的答案正确的信心程度，评分范围为 1 到 4。请按相应的数字键来表示您的信心程度。在您结束指导后，我们将开始练习。</p></div>'
	],
	allow_keys: false,
	...
	loop_function: function(data) {
		...
		if (sumInstructTime <= instructTimeThresh * 1000) {
			feedback_instruct_text =
				'阅读指导太快了。请放慢速度，确保您理解了指导内容。请按 <strong>Enter</strong> 键继续。'
			return true
		} else if (sumInstructTime > instructTimeThresh * 1000) {
			feedback_instruct_text =
				'指导完成。请按 <strong>Enter</strong> 键继续。'
			return false
		}
	}
}
...
var start_test_block = {
	...
	timing_response: 180000,
	text: '<div class = centerbox><p class = center-block-text>练习部分结束。现在我们将开始正式测试。这与练习部分相同 - 您的任务是通过按箭头键指示栅格的位置。在您做出选择后，请评估您的答案准确性的信心程度。</p><p class = center-block-text>按 <strong>Enter</strong> 键继续。</p></div>',
	cont_key: [13],
	timing_post_trial: 0
};
