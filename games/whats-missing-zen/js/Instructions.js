...
Game.Instructions.prototype = {
  ...
  instruct: function (num) {
    if (num == 1) {
          instruct_text = this.game.add.text(this.game.world.centerX-25,460,"在这个游戏中，你的任务是找到完成加法问题的数字。",
          ...
          this.inst_finished = false
          this.back_ground = this.makeBox(instruct_text.x-10,instruct_text.y-10,400,120)
          this.game.world.sendToBack(this.back_ground)
          this.nextB.anchor.x = (0.5,0.5)
          this.nextB.scale.setTo(0.5,0.5)
        } if (num == 2) {
              instruct_text = this.game.add.text(this.game.world.centerX,460,"当你认为知道答案时，\n在数字键盘上输入答案。",
              {font: "20px Arial", fill: "#FFFFFF", align: "center"})
              this.nextB = this.add.button(this.game.world.centerX+175, 530, 'next', function () {
                this.game.world.remove(instruct_text)
                this.rmInstructB = true
              ...
              this.instruct_text = this.game.add.text(this.game.world.centerX+10,500,"现在试试这个问题吧。",
              {font: "20px Arial", fill: "#FFFFFF", align: "center"})
  ...
  nextTrial: function () {
    ...
    this.progress = this.game.add.text(860, 560, this.trial+1 + ' / 1', {font:'30px Arial', fill:'#FFFFFF', align:'center'})
  ...
  grade: function (time_stamp) {
    ...
    if (this.user_answer == correct_answer) {
      ...
      if (this.streak == 2) {
        this.feedback.text = "连续3个正确！奖励金币！"
        this.feedback.fill = '#3CF948'
        //change color to green...
        this.points+=1
      } else if (this.streak == 6) {
        this.feedback.text = "连续7个正确！奖励金币！"
        this.feedback.fill = '#3CF948'
        this.points+=1
      } else if (this.streak == 14) {
        this.feedback.text = "连续15个正确！奖励金币！"
        this.feedback.fill = '#3CF948'
        this.points+=1
      } else if (this.streak == 25) {
        this.feedback.text = "完美得分！奖励金币！"
        this.feedback.fill = '#3CF948'
        this.points+=1
      } else {
        correct_feedback = ['做得好！','太棒了！','你真厉害！','正确！','太出色了！','很好！']
  ...
  quitGame: function (pointer) {
      ...
      this.game.time.events.add(Phaser.Timer.SECOND, function () {
        instructions = this.game.add.text(490, 50, '做得很好！理解了吗？如果理解了，你可以点击“开始”按钮来开始游戏。\n如果没有，你可以点击“返回”按钮重复指示。\n你将在正式游戏中完成 24 道题。', {font:'20px Arial', fill:'#FFFFFF', align:'center'});
