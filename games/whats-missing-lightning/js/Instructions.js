...
Game.Instructions.prototype = {
  ...
  instruct: function (num) {
    if (num == 1) {
          instruct_text = this.game.add.text(this.game.world.centerX-20,460,"在这个游戏中，你的任务是找到\n能够完成每道题目的数字。",
          ...
          this.inst_finished = false
          this.back_ground = this.makeBox(instruct_text.x-5,instruct_text.y-10,400,120)
          this.game.world.sendToBack(this.back_ground)
          this.nextB.anchor.x = (0.5,0.5)
          this.nextB.scale.setTo(0.5,0.5)
    } if (num == 2) {
      instruct_text = this.game.add.text(this.game.world.centerX-10,460,"就像之前的游戏一样，但现在\n你必须在时间用完之前回答。",
              {font: "20px Arial", fill: "#FFFFFF", align: "center"})
              this.nextB = this.add.button(this.game.world.centerX+175, 530, 'next', function () {
                this.game.world.remove(instruct_text)
                this.rmInstructB = true
              ...
              this.nextB.scale.setTo(0.5,0.5)
        		          ...
        		          instruct_text = this.game.add.text(this.game.world.centerX+100,50,"这个圆圈显示你还剩多少时间。",
        		          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
                      ...
                      this.game.world.sendToBack(this.back_ground)
        		          ...
        		          instruct_text = this.game.add.text(this.game.world.centerX+100,50,"真正的游戏会比这个快得多。",
        		          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
        		          this.nextB = this.add.button(this.game.world.centerX+300, 80, 'next', function () {
        		            this.game.world.remove(instruct_text)
        		            this.rmInstructB = true
                  ...
                  instruct_text = this.game.add.text(this.game.world.centerX,470,"当你认为知道答案时，\n在数字键盘上输入它。",
                  {font: "20px Arial", fill: "#FFFFFF", align: "center"})
                  this.nextB = this.add.button(this.game.world.centerX+175, 530, 'next', function () {
                    this.game.world.remove(instruct_text)
                    this.rmInstructB = true
                  ...
                  this.instruct_text = this.game.add.text(this.game.world.centerX,490,"现在试试这个问题吧。",
                  {font: "20px Arial", fill: "#FFFFFF", align: "center"})
  ...
  grade: function (time_stamp) {
    ...
    if (this.user_answer == correct_answer) {
      ...
      if (this.streak == 2) {
        ...
        correct_feedback = ['太棒了！','真厉害！','你很棒！','正确！','太好了！','不错！']
  ...
  quitGame: function (pointer) {
      ...
      this.game.time.events.add(Phaser.Timer.SECOND, function () {
        instructions = this.game.add.text(490, 50, '干得好！明白了吗？如果明白了就按"开始"按钮。\n如果没明白，可以点击"返回"按钮重复说明\n主游戏中你将完成24道题目。\n\n 记住时间会更快！', {font:'20px Arial', fill:'#FFFFFF', align:'center'});
