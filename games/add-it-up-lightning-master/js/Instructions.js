...
Game.Instructions.prototype = {
  ...
  instruct: function (num) {
    if (num == 1) {
          ...
          this.nextB.scale.setTo(0.5,0.5)
		          ...
		          instruct_text = this.game.add.text(this.game.world.centerX,470,"和之前的游戏一样，\n但现在你必须在时间用完之前回答。",
		          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
		          this.nextB = this.add.button(this.game.world.centerX+175, 530, 'next', function () {
		            this.game.world.remove(instruct_text)
		            this.rmInstructB = true
              ...
              this.game.world.sendToBack(this.back_ground)
		          ...
		          instruct_text = this.game.add.text(this.game.world.centerX+100,50,"真正的游戏速度会比这快得多。",
		          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
		          this.nextB = this.add.button(this.game.world.centerX+300, 80, 'next', function () {
		            this.game.world.remove(instruct_text)
		            this.rmInstructB = true
          ...
          instruct_text = this.game.add.text(this.game.world.centerX,470,"当你知道答案时，\n请在数字键盘上输入。",
          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
          this.nextB = this.add.button(this.game.world.centerX+175, 530, 'next', function () {
            this.game.world.remove(instruct_text)
            this.rmInstructB = true
          ...
          this.instruct_text = this.game.add.text(this.game.world.centerX,490,"现在试试这道题吧。",
          {font: "20px Arial", fill: "#FFFFFF", align: "center"})
  ...
  grade: function (time_stamp) {
    ...
    if (this.user_answer == correct_answer) {
      ...
      if (this.streak == 2) {
        ...
        this.feedback.text = "完美分数！额外金币！"
        this.feedback.fill = '#3CF948'
        this.points+=1
      } else {
        correct_feedback = ['太棒了！','真厉害！','你真棒！','正确！','太好了！','不错！']
  ...
  quitGame: function (pointer) {
      ...
      this.game.time.events.add(Phaser.Timer.SECOND, function () {
        instructions = this.game.add.text(490, 50, '做得好！明白了吗？如果明白了，请按"开始"按钮。\n如果不明白，可以点击"返回"按钮重复说明。\n正式游戏中你需要完成24道题。\n\n 记住，正式游戏的速度会快得多！', {font:'20px Arial', fill:'#FFFFFF', align:'center'});
