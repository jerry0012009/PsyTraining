...
var getSelectiveFeedback = function(){
	...
	if (ignoreRespond_percent <= motor_threshold){
          ...
          '<p class = block-text>您一直在对Z和M键都停止反应。请确保<strong>只有在星号出现且您要按'+ stop_response[0]+'时才停止反应。</strong></p><p class = block-text>按<strong>回车键</strong>查看块反馈。'
  	} else {
  		selective_feedback_text =
...
var getTestFeedback = function() {
	...
	test_feedback_text = "<br>测试块完成。请利用这个时间阅读您的反馈并稍作休息！阅读反馈后按<strong>回车键</strong>继续。"
	test_feedback_text += "</p><p class = block-text><strong>平均反应时间：" + Math.round(average_rt) + "毫秒。非星号试验的准确率：" + Math.round(GoCorrect_percent * 100)+ "%</strong>" 
	if (average_rt > RT_thresh || rt_diff > rt_diff_thresh) {
		test_feedback_text +=
			'</p><p class = block-text>您的反应过慢，请对每个形状尽可能快速准确地做出反应。'
	}
	if (missed_responses >= missed_response_thresh) {
		test_feedback_text +=
			'</p><p class = block-text><strong>我们检测到一些需要做出反应的试验中没有任何反应。请确保您对每个形状都做出反应，除非出现星号。</strong>'
	}
	if (GoCorrect_percent < accuracy_thresh) {
		test_feedback_text += '</p><p class = block-text>您的准确率太低。记住，正确的按键如下：' + prompt_text
	}
	if (StopCorrect_percent < (0.5-stop_thresh) || stopAverage < 0.45){
			 	test_feedback_text +=
			 		'</p><p class = block-text><strong>记住，当您看到停止信号且正确按键是' +
					stop_response[0] + '时，请尝试制止您的反应。</strong>' 

	} else if (StopCorrect_percent > (0.5+stop_thresh) || stopAverage > 0.55){
	 	test_feedback_text +=
	 		'</p><p class = block-text><strong>记住，不要为了看是否会出现星号而放慢对形状的反应。请对每个形状尽可能快速准确地做出反应。</strong>'
	}
...
var post_task_block = {
   ...
   questions: ['<p class = center-block-text style = "font-size: 20px">请总结一下您在这个任务中被要求做的事情。</p>',
...
var instructions_block = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>在这个任务中，你将看到黑色形状逐一出现在屏幕上。你需要通过按"Z"和"M"键来对它们做出反应。</p></div>',
		'<div class = centerbox><p class = block-text>每个形状只有一个正确的按键。正确的按键如下：' + prompt_text +
		'</p><p class = block-text>这些指导说明将在练习阶段保留在屏幕上，但在测试阶段会被移除。</p><p class = block-text>你应该对每个形状尽可能快速准确地做出反应。</p></div>',
	],
	allow_keys: false,
	...
	loop_function: function(data) {
		...
		if (sumInstructTime <= instructTimeThresh * 1000) {
			feedback_instruct_text =
				'阅读指导说明太快。请花时间仔细阅读，确保您理解指导说明。按<strong>回车键</strong>继续。'
			return true
...
var practice_feedback_text =
	'我们现在开始练习阶段。在练习中，请专注于对每个形状快速准确地做出反应。按<strong>回车键</strong>继续。'
...
var NoSS_practice_node = {
	...
	loop_function: function(data) {
		...
		var GoCorrect_percent = sum_correct / go_length;
		var missed_responses = (go_length - num_responses) / go_length
		practice_feedback_text = "</p><p class = block-text><strong>平均反应时间：" + Math.round(average_rt) + "毫秒。非星号试验的准确率：" + Math.round(GoCorrect_percent * 100)+ "%</strong>" 
		if ((average_rt < RT_thresh && GoCorrect_percent > accuracy_thresh && missed_responses <
			...
			practice_feedback_text +=
				'</p><p class = block-text>在实验的剩余部分，在一定比例的试验中，形状周围会出现星形的黑色"停止信号"。当这种情况发生时，如果该试验的正确按键是' +
					stop_response[0] + '，请尽力停止您的反应，在该试验中不按任何键。</p><p class = block-text>星号会在形状出现的同时或稍后出现。因此，当星号出现时，您不会总是能够成功停止。然而，如果您继续非常努力地在星号出现时停止，您有时能够停止，但并非总是如此。</p><p class = block-text>如果正确反应不是' + stop_response[0] + '，请像平常一样反应。</p><p class = block-text><strong>请平衡快速准确地对形状做出反应的要求，同时非常努力地对停止信号停止反应。</strong></p><p class = block-text>按<strong>回车键</strong>继续'
			return false;
		} else {
			//rerandomize stim order
			NoSS_practice_list = jsPsych.randomization.repeat(stimuli, 3, true)
			// keep going until they are faster!
			practice_feedback_text += '</p><p class = block-text>我们将进行另一个练习块。'
...
var practice_node = {
	...
	loop_function: function(data) {
		...
		if ((average_rt < RT_thresh && GoCorrect_percent > accuracy_thresh && missed_responses <
			...
			current_trial = 0
			practice_feedback_text +=
				'</p><p class = block-text>练习完成。我们现在将开始' + numblocks +
				'个测试块。每个块后都会有休息。按<strong>回车键</strong>继续。'
			return false;
		} else {
			...
			if (average_rt > RT_thresh) {
				practice_feedback_text +=
					'</p><p class = block-text>反应过慢。记住，请尽可能快速准确地做出反应。'
			}
			if (missed_responses >= missed_response_thresh) {
				practice_feedback_text +=
					'</p><p class = block-text>错过太多反应。记住对每个形状做出反应，除非您看到黑色停止信号且正确按键是' + stop_response[0] + '。'
			}
...
motor_selective_stop_signal_experiment.push(post_task_block)
